 ...
 * semantic events emitted to [TemporalEventStore].
 *
 * Semantic Rules:
 *  1. [EventType.PERSON_ENTERED_VIEW]: Emitted ONCE (Source: VISION) when a new `trackId` is first observed.
 *  2. Movement Transitions: Emitted ONLY when stable movement state changes (Source: FUSED).
